...
function isSignal(hour, minutes, seconds, p){
	...
	// console.log((pcp));
	// console.log('---------------------------');
	if(((pcp) + 5)<= 0.5 && (ct).Buy_CountI >= count && p >= power && ((plp) - (pcp)) < 2){
		return {signal: true} 
	}
...
if (hour >= 9 && hour <= 12 && (minutes % 3 === 0))
{
	if(hour == 12 && minutes > 45){
		return false;
	}
	if (buyEachPerson >= 20000000 && (((pc) < (pl) && buyOnSell >= 1.2) || buyOnSell >= 1.5) && (pl)!=(tmax))
	{	
		if([ih] != undefined){
